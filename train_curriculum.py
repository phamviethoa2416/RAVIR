"""
Curriculum Learning for Retinal Vessel Segmentation.

Round 1 — Vessel Discovery:
    Train binary vessel extractor from scratch on merged
    DRIVE + STARE + CHASE_DB1 (RGB colour fundus images).

Round 2 — Domain Adaptation:
    Fine-tune on RAVIR infrared images (binary vessel task)
    starting from the Round 1 checkpoint.

Usage:
    python train_curriculum.py --round 1 --epochs 150
    python train_curriculum.py --round 2 --checkpoint outputs/curriculum/round1/round1_best.pth
"""
...
from training import train_one_epoch_binary, validate_binary, get_amp_dtype
...
def adapt_state_dict_channels(
    ...
    """Adapt the encoder's first conv from *src_channels* → *tgt_channels*."""
...
def train_round1(args):
    ...
    # ── Data ──────────────────────────────────────────────────────────
    curriculum_root = os.path.join(Config.DATA_DIR, "curriculum")
    patch_size = args.patch_size

    train_dataset, val_dataset = build_curriculum_datasets(
...
# ═════════════════════════════════════════════════════════════════════════════
#  Round 2 — Domain Adaptation (RAVIR IR, Binary)
# ═════════════════════════════════════════════════════════════════════════════
